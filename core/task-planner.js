...
const MAX_STEPS = 8;
// Only genuinely sensitive desktop operations need an approval gate.
// Opening an allowlisted app/path/URL is intentionally frictionless for voice use.
const APPROVAL_TOOLS = Object.freeze([
  'mouse_click',
  'type_text',
  'capture_screen',
  'analyze_screen',
...
function splitTask(text) {
  const value = String(text || '').trim();
  if (!value) return [];
  return value.split(/\s+(?:and then|then|after that)\s+|\s*[,;]\s*/i).map((part) => part.trim()).filter(Boolean);
}
...
function planTask(text) {
  ...
  return {
    ...
    requiresApproval: steps.some((step) => APPROVAL_TOOLS.includes(step.tool)),
