...
const COMMAND_WORDS = Object.freeze({ open: ['open', 'launch', 'start', 'khol', 'kholo', 'khol do', 'खोल', 'खोलो'], show: ['show', 'dikha', 'dikhao', 'दिखा', 'दिखाओ'], go: ['go to', 'visit', 'jao', 'जाओ'] });
...
function extractWebsite(q) {
  const match = q.match(/(?:open|go to|visit|browse|khol|kholo|जाओ|खोलो)\s+(?:the\s+)?(?:website\s+)?((?:https?:\/\/)?(?:www\.)?[a-z0-9][a-z0-9.-]*\.[a-z]{2,}(?:\/[^\s]*)?)/i);
  return match ? match[1] : null;
}
function extractGenericApp(q) {
  const match = q.match(/^(?:open|launch|start|khol|kholo|khol do)\s+(?:the\s+)?(?:app\s+)?(.+?)\s*$/i);
  if (!match) return null;
  const candidate = match[1].trim().replace(/\s+(?:app|application)$/i, '').trim();
  if (!candidate || /^(?:my\s+)?(?:desktop|documents|downloads|screen)$/i.test(candidate)) return null;
  if (extractWebsite(q)) return null;
  return candidate;
}
function planLocalCommand(text) {
  ...
  const arbitraryWebsite = extractWebsite(q);
  if (arbitraryWebsite) return makeTool('open_url', { url: arbitraryWebsite }, `Open ${arbitraryWebsite} in your default browser.`, `Open ${arbitraryWebsite}`);
  ...
  const arbitraryApp = extractGenericApp(q);
  if (arbitraryApp) return makeTool('open_app', { app: arbitraryApp }, `Find and launch the installed application “${arbitraryApp}”.`, `Open ${arbitraryApp}`);
...
module.exports = { planLocalCommand, parseShortcut, KEY_ALIASES, COMMAND_WORDS, hasAny, extractWebsite, extractGenericApp };
